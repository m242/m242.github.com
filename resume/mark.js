;$(function() {
	...
	showTweets = function($div, word, f) {
		$.getJSON("http://search.twitter.com/search.json?rpp=10&q=" + encodeURIComponent(word) + "&result_type=recent&geocode=37.781157,-122.398720,25mi&callback=?", function(data) {
			var results, i, tweet, html, text, from_user_name;
			results = data.results;
			for(i=0;i<results.length;i++) {
				tweet = results[i];
				text = tweet.text;
				text = text.replace(new RegExp(word, "i"), "<em>" + word + "</em>");
				from_user_name = tweet.from_user_name.replace(new RegExp(word, "i"), "<em>" + word + "</em>");
				html = "<div class=\"item " + (i == 0 ? "active" : "") + "\"><span class=\"profile_image_url\"><img class=\"profile\" src=\"" + tweet.profile_image_url + "\" width=\"48\" height=\"48\"></span>" +
